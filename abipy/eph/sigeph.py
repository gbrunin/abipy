...
class SigEPhFile(AbinitNcFile, Has_Structure, Has_ElectronBands, NotebookWriter):
    ...
    def get_lifetimes_boltztrap(self,basename):
        ...
        def write_file(filename,tag,function,T=None):
            """Funciton to write files for BoltzTraP"""
        ...
        #write structure
        structure = self.ebands.structure 
        fmt3 = "%20.12e "*3 + '\n'
        with open(basename+'_BLZTRP.structure','w') as f:
            f.write('BoltzTraP geometry file generated by abipy.\n')
            f.write(fmt3%tuple(structure.lattice.matrix[0]*Ang_Bohr))
            f.write(fmt3%tuple(structure.lattice.matrix[1]*Ang_Bohr))
            f.write(fmt3%tuple(structure.lattice.matrix[2]*Ang_Bohr))
            f.write("%d\n"%len(structure))
            for atom in structure:
                f.write("%s "%atom.specie+fmt3%tuple(atom.coords*Ang_Bohr))
